...
const FourCsFramework = () => {
  ...
  const frameworks = {
    computation: {
      ...
      description: "Computation names how generative systems assemble meaning through probability rather than understanding. Each output is a statistical event—patterns of language weighted and recombined until they appear coherent. To read computation is to trace how repetition, frequency, and exclusion shape what can be said. It reveals the architectures of knowing, and how both machines and humans can miscalculate through the limits of their data and the patterns they mistake for truth.",
      comparisons: [
        {
          process: "Pattern Recognition",
          icon: "network",
          reflection: "Prediction arises from repetition. A large language model generates text by estimating the most probable next token, based on weighted distributions learned across large textual corpora. Its knowledge is statistical rather than semantic: proximity, not comprehension.\n\n<span class=\"font-bold\" style=\"color: #9BB8E0\">This means we have to ask: what patterns is the system most likely to have learned, and whose language, values, and histories those patterns represent.</span>"
        },
        {
          process: "Boundaries of Knowing",
          icon: "borders",
          reflection: "Every model knows within the borders of its data. Its corpus is finite, delimited by what was gathered, filtered, or forbidden. These absences define the contours of its world—the unspoken edges where the unsayable resides.\n\n<span class=\"font-bold\" style=\"color: #9BB8E0\">This means we should ask what is overrepresented and what is missing in the data that defines the model's world—and how those boundaries shape what it can produce.</span>"
        },
        {
          process: "Centring and the Normative",
          icon: "target",
          reflection: "A model privileges the central tendency. Its fluency arises from averaging—the smooth convergence of what is most frequent. Marginal, resistant, or rare expressions are statistically diluted until they sound more like the middle.\n\n<span class=\"font-bold\" style=\"color: #9BB8E0\">This means we need to learn how to direct the model away from the middle—toward the edges of its data where difference and possibility live. We can't fully escape its statistical gravity, but we can learn to recognize and work with it: adjusting prompts, reading for variance, and seeking what resists the average.</span>"
        },
        {
          process: "Inheritance and History",
          icon: "archive",
          reflection: "A model inherits its world. It speaks from the sediment of past data—temporal cut-offs, linguistic hierarchies, web visibility. Its sense of reality is the archive rendered probabilistic: what was most recorded, not what was most real.\n\n<span class=\"font-bold\" style=\"color: #9BB8E0\">This means we need to learn to read not for intent, but for statistical positionality: to recognize how a model's \"voice\" is shaped by the distributions of what has been most recorded, repeated, or excluded. It does not possess consciousness, yet it manifests a patterned worldview.</span>"
        }
      ],
      whyMatters: "Computation reveals the material, statistical, and historical constructed features of meaning. For educators, this reframes literacy as: an inquiry into how representation is distributed rather than merely what it states; an opportunity to explore how data and discourse co-produce knowledge; a bridge between numeracy and interpretation, where pattern becomes a shared analytic lens.",
      ...
      questionsForHumans: "What patterns do I trust, and why? — When does familiarity start to feel like truth?\n\nWhat remains outside my data? — What voices, examples, or histories are missing from how I know?\n\nHow am I averaging the world? — In seeking coherence, what difference or uncertainty do I smooth away?",
      ...
      comparisons: [
        {
          process: "Sequencing",
          icon: "archive",
          reflection: "Generative systems compose sequentially. Each element—word, pixel, or note—is chosen in response to what came immediately before, guided by probability rather than plan. The model writes forward only, predicting what is most likely to follow without reconsidering what it has already produced. There is no outline, no revision cycle, no vision of the whole—only the continuous calculation of what fits next.\n\n<span class=\"font-bold\" style=\"color: #E89B5F\">This means we risk mistaking movement for direction—following fluency itself wherever it leads. The task is to notice this pull rather than be carried away by it.</span>"
        },
        {
          process: "Scaffolding",
          icon: "borders",
          reflection: "Every act of generation begins within a frame. A prompt, instruction, or prior example sets the boundaries of what can be said and how it will sound. The model composes by conforming to that scaffolding, shaping its next prediction to match the tone, format, and logic it infers from the given structure.\n\n<span class=\"font-bold\" style=\"color: #E89B5F\">This means we must ask how our own frames—our prompts, genres, and expectations—scaffold what can appear, and what remains unsayable within the structures we create.</span>"
        },
        {
          process: "Layering",
          icon: "target",
          reflection: "As the model moves forward, it builds meaning through accumulation. Each new token is influenced by those still visible within its context window, where attention gradually fades with distance. Earlier segments may subtly shape the present, but they are never revised. The machine's layering makes visible how sense depends on what persists in attention and what quietly drops away.\n\n<span class=\"font-bold\" style=\"color: #E89B5F\">This means we must ask what sits inside the context window, what lingers at its edges, and whether the machine—like the mind—needs more layers or fewer to sustain coherence in what it composes.</span>"
        },
        {
          process: "Style as Algorithm",
          icon: "network",
          reflection: "Style is the trace left by the model's compositional process. It arises from the statistical inheritance of its training data—the collective rhythm, tone, and phrasing patterns that have proven most probable across its finite corpus. What reads as personality is really the residue of optimization. To read AI style (or its lack) is to read a smoothed record of uneven cultural forms—how they have been averaged, tuned, and aligned. It reminds us that style itself, human or machinic, is never free of training.\n\n<span class=\"font-bold\" style=\"color: #E89B5F\">What this means is that AI \"slop\" is what happens when optimization exceeds intention—when pattern replaces voice and coherence becomes its own aesthetic. It has become a style in itself—an algorithmic aesthetic where probability stands in for intention.</span>"
        }
      ],
      ...
      questionsForMachine: [
        {
          question: "Filtering",
          icon: "borders",
          answer: "Jess Smith's prompt—\"Make me an image of a woman like me\"—reveals how filtering quietly governs what can appear. Long before generation, automated systems remove or down-rank images deemed violent, explicit, or disturbing, and often over-flag depictions of bodily difference as \"sensitive.\" Disabled and non-normative bodies are already rare in the public image archives from which models learn. The result is a world rebuilt through omission—a biased system in which the different is rendered impossible.",
          recalibrate: "What would it take to design a model that learns from, rather than erases, difference?"
        },
        {
          question: "Tone Smoothing",
          icon: "target",
          answer: "When systems reply with calm explanations—\"I don't have enough data to work with\"—they do so in a voice trained to be agreeable. Models are tuned to sound polite, balanced, and nonconfrontational, even when their failures reproduce harm. This smoothing of tone performs care by muting discomfort. Conflict dissolves into civility; harm is flattened into help.",
          recalibrate: "How might we prompt a system to hold discomfort instead of resolving it—to speak from the tension, not past it?"
        },
        {
          question: "Consensus Formation",
          icon: "network",
          answer: "Generative systems predict what is most likely to appear, based on what has appeared most often. Across billions of images, the statistical \"average\" body dominates, while others fade from view. This is not malfunction but fidelity to probability itself: the mathematics of the majority rendered as truth. When we ask for a person \"like me,\" the model returns the person most often seen.",
          recalibrate: "How might a model be built to recognize the rare instead of the common, to generate against, rather than toward, the average?"
        },
        {
          question: "Refusal",
          icon: "archive",
          answer: "When a system says it \"can't\" create an image, that boundary is both technical and political. Refusals mark the edges of a model's training data and the values embedded in its safety rules. They signal where the system's world ends—and whose worlds have been left out. A refusal is not silence but a message about structure.",
          recalibrate: "What if a model could explain its refusals—showing not only that it cannot speak, but why that silence exists and whose safety it serves?"
        }
      ],
    ...
    calibration: {
      ...
      examplePrompt: "Help me understand a passage in this difficult text.",
      exampleContext: "The model responds fluently, paraphrasing the passage in clear, accessible prose. It sounds confident, even illuminating. Yet the more complex or ambiguous the original text, the more its explanation risks flattening what makes it difficult. The system predicts coherence where it may not exist, filling gaps in understanding with patterns of familiarity. To read with calibration is to learn to hear this smoothness as signal—to ask how certainty, error, and doubt circulate in the model's performance of comprehension.",
      questionsForMachine: [
        {
          question: "Confidence Signaling",
          icon: "target",
          answer: "When asked to explain a difficult text, the model performs understanding through tone. Words like clearly, in essence, or what the author means convey assurance even when the text resists easy meaning. Confidence becomes a linguistic gesture, not evidence of insight. The model translates ambiguity into clarity because clarity is what its training rewards.",
          recalibrate: "What happens if we ask it to preserve the uncertainty instead—to explain what might be unclear or contested, rather than pretending the passage can be fully known?"
        },
        {
          question: "Variance and Error",
          icon: "network",
          answer: "If we prompt the same request twice, the model's interpretations shift: a metaphor becomes a moral, a critique turns descriptive, tension dissolves. These differences are not random—they trace where meaning exceeds pattern recognition. The model improvises coherence, assembling the most probable reading from textual echoes it has seen before.",
          recalibrate: "What might we learn by comparing these versions side by side—treating the differences not as error, but as maps of interpretive possibility?"
        },
        {
          question: "Trust and Verification",
          icon: "borders",
          answer: "The system's eloquence can make its reading feel authoritative, yet trust must be earned through engagement, not style. Paraphrase can obscure as much as it clarifies; simplification can distort nuance. Calibration here means reading the model's gloss against the text itself—testing where it aligns, where it diverges, and where fluency substitutes for thought.",
          recalibrate: "How might we use the model's response as a lens for rereading the original—letting its misreadings reveal what we ourselves overlooked?"
        },
        {
          question: "Meta-Awareness",
          icon: "archive",
          answer: "When the model admits, \"This passage is complex and open to interpretation,\" it mimics humility but cannot sense its own uncertainty. Humans can. True calibration means noticing when we too are drawn to what sounds clear, when we accept smoothness as understanding. Working with generative systems can sharpen this awareness—teaching us to read not only for meaning, but for the performance of meaning.",
          recalibrate: "How might we design prompts, or pedagogies, that keep uncertainty visible—where not knowing becomes part of how we learn to read?"
        }
      ],
                  ...
                  <div className="bg-slate-700/50 rounded-lg overflow-hidden">
                    ...
                    {expandedQuestions[framework.type] && (
                      ...
                      {framework.examplePrompt && (
                        <div className="bg-purple-900/20 rounded-lg p-5 mb-4 space-y-3 border border-purple-500/20">
                          <div>
                            <span className="text-sm font-bold text-purple-300 uppercase tracking-wide">Prompt:</span>
                            <p className="text-gray-200 text-base mt-1 italic">"{framework.examplePrompt}"</p>
                          </div>
                          {framework.exampleResponse && (
                            <div>
                              <span className="text-sm font-bold text-purple-300 uppercase tracking-wide">AI Response:</span>
                              <p className="text-gray-200 text-base mt-1 italic">"{framework.exampleResponse}"</p>
                            </div>
                          )}
                              ...
                              {typeof q === 'string' ? (
                                ...
                                <div className="space-y-2 bg-slate-800/30 rounded-lg p-3">
                                  ...
                                  <div className="pt-2 border-t border-slate-700/50">
                                    <p className="text-teal-400 mt-1 italic">{q.recalibrate}</p>
                                  </div>
                                </div>
                              ) : (
                      ...
                      <div className="bg-slate-700/40 rounded-lg p-5 border border-slate-600/50">
                        <h5 className="text-base font-semibold text-teal-300 mb-3 uppercase tracking-wide">
                          Ask Ourselves
                        </h5>
                        {typeof framework.questionsForHumans === 'string' ? (
                          ...
                          <div className="space-y-3">
                            {framework.questionsForHumans.split('\n\n').map((paragraph, idx) => (
                              <p key={idx} className="text-gray-200 text-base leading-relaxed italic">{paragraph}</p>
                            ))}
                          </div>
                        ) : (
            ...
            <div className="px-8 pb-8 space-y-6 text-gray-300 leading-relaxed">
              <div className="space-y-4">
                <p className="text-sm text-gray-300 leading-relaxed">
                  Generative AI systems are now deeply embedded in how knowledge is produced, circulated, and judged. Yet our pedagogies and interpretive habits—across text, image, and sound—have not caught up. We keep reading AI outputs as if they belonged to older genres of meaning as: true or false, honest or deceitful, authentic or artificial, to be verified or dismissed, or as authored expression to be read for intention.
                </p>

                <p className="text-sm text-gray-300 leading-relaxed">
                  What these framings miss is what is most distinct about generative systems: that meaning, here, is computed rather than conceived.
                </p>

                <p className="text-sm text-gray-300 leading-relaxed">
                  A computational approach to sense-making builds on <span className="font-bold text-teal-400">Jeannette Wing's (2006)</span> influential idea of <span className="font-bold italic text-teal-400">computational thinking</span>, first proposed as a way to communicate with computers and to bring that conversation beyond the computer science classroom. Wing positioned <span className="font-bold italic">computational thinking</span> as a fundamental literacy for everyone—on par with reading, writing, and arithmetic—drawing on the core concepts of computer science to solve problems, design systems, and understand human behaviour.
                </p>

                <p className="text-sm text-gray-300 leading-relaxed">
                  Now, however, across disciplines, we are engaged in intimate conversations with computers—regardless of our technical proficiency, yet we rarely attend to their computational nature. Large language models have made computers feel infinitely more accessible—fluent, conversational, almost human—yet this very fluency conceals the operations through which they compute. The 4Cs framework extends Jeannette Wing's call for computational literacy into the interpretive domain: not only how we build with computation, but <span className="font-bold text-teal-400">how we read computation when it speaks</span>. It invites us to remember what these systems are doing when they seem to understand us—and, equally, what we are doing when we seek to understand them.
                </p>
              </div>
